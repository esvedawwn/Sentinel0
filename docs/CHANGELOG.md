...
## [v0.7.5-alpha] — 2026-07-14 — Fix shell plugin config + database initialisation order

### Fixed

#### `artifacts/desktop/src-tauri/tauri.conf.json` — confirmed clean, build guard added
- Source config already had `plugins.shell` removed (v0.7.4 fix).  The launch
  crash reported after that fix was caused by running a stale installed binary
  built before that commit.
- Added `artifacts/desktop/scripts/check-tauri-config.mjs`: a pre-build
  validator that scans every `tauri.*.conf.json` and `capabilities/*.json` for
  `"all"` under `plugins.shell`, fails with exit code 1 if found, and is
  wired into `pnpm desktop:build` so the invalid field can never silently
  survive a future merge.

#### `artifacts/api-server/src/index.ts` — DB initialisation before first query
- **Root cause**: on a brand-new installation the SQLite file is empty — no
  tables exist.  The startup IIFE immediately ran
  `db.update(scansTable).where(...)` (`cleanupInterruptedScans`) before any
  schema had been applied, producing `SQLITE_ERROR: no such table: scans`.
- **Fix**: `runMigrations(client)` is now called as step 1 of the startup IIFE,
  before the cleanup step and before `app.listen()`.  Migration failure is fatal
  and logged; an empty database is not treated as an error.

#### `lib/db/src/migrate.ts` + `lib/db/src/migrations-snapshot.ts` — new files
- `runMigrations(client)` checks `sqlite_master` for the `scans` table.  If
  absent (first launch), it applies all 75 DDL statements from the generated
  snapshot in a single deferred (atomic) batch via `client.batch()`.
  If the table exists it returns immediately — safe to call on every startup.
- `migrations-snapshot.ts` is generated by `lib/db/scripts/codegen-migrations.mjs`
  from the Drizzle-generated SQL in `lib/db/drizzle/`.  The SQL is inlined as a
  TypeScript string array so no external files are needed at runtime inside the
  `@yao-pkg/pkg` binary.
- Regenerate after schema changes:
  ```bash
  pnpm --filter @workspace/db run codegen:migrations
  ```

#### `artifacts/api-server/src/routes/__tests__/startup.test.ts` — new test file
- 4 new tests verify: migration runs on a blank DB, is idempotent, cleanup
  succeeds on an empty DB, and `/api/healthz` returns 200 — all without
  `drizzle-kit push`, exercising the exact code path used in the packaged binary.
