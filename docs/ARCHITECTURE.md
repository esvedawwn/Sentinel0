...
Background: runRealScan(scanId, path, isSample=true)
        ...
        │
        └── DB writes (findings table, scan progress updates)
```
...
| Build (API) | esbuild | Fast CJS bundle |
...
- **`realScanner.ts`** — orchestrator: ties DB writes, progress updates, and the two modules above
